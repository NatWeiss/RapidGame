...
var MenuScene = function(window, game) {
	...
	var onSceneActivated = function(e) {
		...
		// Exit label
		exitLabel = platino.createTextSprite({
			text: "Exit",
			fontSize: 120,
			fontFamily: "Dolce Vita",
			alpha: 0.333
		});
		size = exitLabel.sizeWithText(exitLabel.text);
		exitLabel.center = {x: middle.x - size.width * .5 + xOffset, y: middle.y - size.height * .5 + yOffset};
		scene.add(exitLabel);
	...
	scene.onTouch = function(event) {
		...
		if (playLabel.contains(touchX, touchY)) {
		...
		} else if (exitLabel.contains(touchX, touchY)) {
			exitLabel.color(1, 0, 0);
			labelTransform = platino.createTransform({
				duration: 200,
				scaleX: 1.2,
				scaleY: 1.2,
			...
			exitLabel.transform(labelTransform);
			setTimeout(function(){
				exitLabel.color(0, 0, 0);
				//removeListeners();
				//game.stop();
			}, 400);
		}
	...
	var onSceneDeactivated = function(e) {
		...
		remove(exitLabel); exitLabel = null;
